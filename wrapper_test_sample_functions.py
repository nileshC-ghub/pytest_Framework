...
#file_path='D:\\Software\\Python\\python_git\\src\\pytest_Framework\\'
...
for i in emplst['Emp_id']:
    if (path.exists(str(file_path+'sample_functions_'+str(i)+'.py'))):
        pytest.main(['-v',str(file_path+'test_sample_functions.py'),'--ifile',str('sample_functions_'+str(i)),'--csv','tests_result.csv','--csv-columns','id,module,name,file,doc,markers,status,message,duration'])
        testdf=pd.read_csv('tests_result.csv')
        testdf['Emp_id']=[str(i)]*len(testdf.index)
        #print([str(i)]*len(testdf.index))
        redf.append(testdf)
    else:
        print(str(file_path+'sample_functions_'+str(i)+'.py')+' file does not exists')
